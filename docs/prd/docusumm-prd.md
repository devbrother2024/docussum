...
2. "크레딧이 부족합니다. 추가 크레딧을 충전하세요"라는 알림이 표시된다.
...
- [ ] **Story 4.1: Stripe 프로젝트 설정 및 상품 등록**
  - Stripe 개발자 계정 설정 및 API 키(Test/Live) 환경변수 구성
  - Stripe Dashboard에 3가지 크레딧 패키지 상품 등록 ($5/30, $8/50, $15/100)
  - Webhook 수신을 위한 Endpoint 보안 설정 (Webhook Secret)
- [ ] **Story 4.2: 결제 UI 및 Checkout 세션 생성**
  - 사용자 대시보드 내 '충전하기' 모달 또는 페이지 UI 구현
  - 패키지 선택 시 Server Action을 통해 Stripe Checkout Session 생성
  - Stripe 결제 페이지로 리다이렉션 및 `metadata`에 사용자 ID/크레딧 정보 주입
- [ ] **Story 4.3: Webhook 처리 및 크레딧 지급 로직**
  - Next.js API Route (`/api/webhooks/stripe`) 생성 및 원본(Raw) Body 파싱 설정
  - `checkout.session.completed` 이벤트 수신 시 서명 검증
  - 검증 성공 시 `users` 테이블의 크레딧 잔액 업데이트 (트랜잭션 안전성 보장)
- [ ] **Story 4.4: 결제 피드백 및 예외 처리**
  - 결제 성공(`success_url`) 및 취소(`cancel_url`) 페이지 또는 상태 처리
  - 결제 성공 시 사용자에게 실시간 토스트 알림 및 크레딧 UI 갱신
  - Webhook 처리 실패 시 재시도 메커니즘 또는 로그 기록
